...
export async function cancelStudentScheduleItemById(scheduleId: string, reason = ""): Promise<StudentScheduleItem> {
  const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL?.trim();
  if (!baseUrl) {
    throw new Error("NEXT_PUBLIC_API_BASE_URL is not configured.");
  }

  const response = await browserApiRequest<StudentScheduleApiItem>({
    url: `${baseUrl}/student/schedule/${encodeURIComponent(scheduleId)}/cancel`,
    method: "PATCH",
    data: { reason },
  });

  return mapStudentScheduleItem(response);
}
