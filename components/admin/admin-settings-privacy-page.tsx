...
import { FiBold, FiEdit2, FiItalic, FiLink2, FiList, FiPaperclip, FiTrash2, FiUnderline } from "react-icons/fi";

import { AdminSettingsLayout } from "@/components/admin/admin-settings-layout";
import {
  fetchAdminPrivacySettings,
...
export function AdminSettingsPrivacyPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const response = await fetchAdminPrivacySettings();
        if (cancelled) return;
  ...
  const onSave = async () => {
    ...
    try {
      const response = await updateAdminPrivacySettings({
        page: {
          title,
  ...
  return (
    <AdminSettingsLayout
      title={title}
      subtitle="Set privacy & policy of your SaaS app"
    ...
    >
      ...
      <div className="mt-3 flex items-center justify-end gap-2">
        {loading ? <p className="mr-auto text-[14px] text-[#6b7280]">Loading privacy settings...</p> : null}
